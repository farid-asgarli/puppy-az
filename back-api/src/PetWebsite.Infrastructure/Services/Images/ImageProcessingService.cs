	...
	: IImageProcessingService
{
	...
	private void ApplyWatermark(SKBitmap bitmap, string text)
	{
		...
		using var shadowPaint = new SKPaint { Color = new SKColor(0, 0, 0, 65), IsAntialias = true };
		canvas.DrawText(text, cx - textWidth / 2f + 1f, cy + fontSize / 3f + 1f, font, shadowPaint);

		// White semi-transparent text (~33 % opacity)
		using var paint = new SKPaint { Color = new SKColor(255, 255, 255, 85), IsAntialias = true };
