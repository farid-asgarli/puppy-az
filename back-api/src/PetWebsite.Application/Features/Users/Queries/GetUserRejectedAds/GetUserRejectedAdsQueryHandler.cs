...
public class GetUserRejectedAdsQueryHandler(
	IApplicationDbContext dbContext,
	ICurrentUserService currentUserService,
	IDynamicQueryRepository queryRepo,
	IUrlService urlService,
	IStringLocalizer localizer
) : BaseHandler(localizer), IQueryHandler<GetUserRejectedAdsQuery, Result<PaginatedResult<MyPetAdListItemDto>>>
{
	public async Task<Result<PaginatedResult<MyPetAdListItemDto>>> Handle(GetUserRejectedAdsQuery request, CancellationToken ct)
	{
		...
		// Get user's rejected ads (status = Rejected)
		var query = dbContext
			.PetAds.WhereNotDeleted<PetAd, int>()
			.AsNoTracking()
			.Where(p => p.UserId == userId && p.Status == PetAdStatus.Rejected)
