import { PetAdType, PetGender, PetSize, type MappedPetAdType } from "@/lib/api";
import {
  IconHeart,
  IconHeartBroken,
  IconPaw,
...
// Icon mapping for ad types by key
const AD_TYPE_ICONS: Record<string, typeof IconHeart> = {
  sale: IconReplaceUser,
  found: IconRadar,
  lost: IconHeartBroken,
  match: IconPaw,
  owning: IconHeart,
};

/**
 * Get icon component for an ad type
 */
export const getAdTypeIcon = (key: string) => AD_TYPE_ICONS[key] || IconPaw;
...
/**
 * @deprecated Use useAdTypes() hook instead to get database-driven ad types
...
/**
 * Convert database ad types map to the format expected by components
 * This adds icon component to each type
 */
export function mapAdTypesWithIcons(
  adTypesMap: Record<string, MappedPetAdType>,
) {
  const result: Record<string, MappedPetAdType & { icon: typeof IconHeart }> =
    {};

  for (const [key, type] of Object.entries(adTypesMap)) {
    result[key] = {
      ...type,
      icon: getAdTypeIcon(key),
    };
  }
    ...
    [PetGender.Unknown]: {
      ...
      icon: (
        ...
        >
          ...
          <path
            d="M12 6v-2m0 16v-2m6-6h2m-16 0h2"
            strokeWidth="2"
            strokeLinecap="round"
