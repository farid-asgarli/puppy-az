...
interface SignalRContextType {
  ...
  // Subscribe to events
  onNewMessage: (
    handler: NotificationHandler<NewMessageNotification>,
  ) => () => void;
  onConversationMessage: (
    handler: NotificationHandler<ConversationMessageNotification>,
  ) => () => void;
  onNewQuestion: (
    handler: NotificationHandler<NewQuestionNotification>,
  ) => () => void;
  onQuestionAnswered: (
    handler: NotificationHandler<QuestionAnsweredNotification>,
  ) => () => void;
  onNewReply: (
    handler: NotificationHandler<NewReplyNotification>,
  ) => () => void;
  onQuestionDeleted: (
    handler: NotificationHandler<QuestionDeletedNotification>,
  ) => () => void;
  onReplyDeleted: (
    handler: NotificationHandler<ReplyDeletedNotification>,
  ) => () => void;
  onAnswerDeleted: (
    handler: NotificationHandler<AnswerDeletedNotification>,
  ) => () => void;
  onUnreadCountUpdate: (
    handler: NotificationHandler<UnreadCountNotification>,
  ) => () => void;
...
export function SignalRProvider({ children }: { children: ReactNode }) {
  ...
  const handlersRef = useRef<{
    ...
    conversationMessage: Set<
      NotificationHandler<ConversationMessageNotification>
    >;
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      try {
        ...
        const hubUrl = `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5005"}/hubs/notifications`;
        ...
        connection.on(
          ...
          (notification: NewMessageNotification) => {
            ...
            handlersRef.current.newMessage.forEach((handler) =>
        ...
        connection.on(
          ...
          (notification: ConversationMessageNotification) => {
        ...
        connection.on(
          ...
          (notification: NewQuestionNotification) => {
            ...
            toast(
              `❓ ${notification.questionerName} "${notification.petAdTitle}" elanınıza sual yazdı`,
              {
                duration: 5000,
                position: "top-right",
                style: {
            ...
            handlersRef.current.newQuestion.forEach((handler) =>
              handler(notification),
            );
          },
        );
        ...
        connection.on(
          "ReceiveQuestionAnswered",
          (notification: QuestionAnsweredNotification) => {
            ...
            handlersRef.current.questionAnswered.forEach((handler) =>
              handler(notification),
            );
          },
        );
        ...
        connection.on(
          ...
          (notification: NewReplyNotification) => {
            ...
            handlersRef.current.newReply.forEach((handler) =>
              handler(notification),
            );
          },
        );
        ...
        connection.on(
          "ReceiveQuestionDeleted",
          (notification: QuestionDeletedNotification) => {
            console.log("[SignalR] ReceiveQuestionDeleted:", notification);
            handlersRef.current.questionDeleted.forEach((handler) =>
              handler(notification),
            );
          },
        );
        ...
        connection.on(
          "ReceiveReplyDeleted",
          (notification: ReplyDeletedNotification) => {
            console.log("[SignalR] ReceiveReplyDeleted:", notification);
            handlersRef.current.replyDeleted.forEach((handler) =>
              handler(notification),
            );
          },
        );
        ...
        connection.on(
          "ReceiveAnswerDeleted",
          (notification: AnswerDeletedNotification) => {
            console.log("[SignalR] ReceiveAnswerDeleted:", notification);
            handlersRef.current.answerDeleted.forEach((handler) =>
              handler(notification),
            );
          },
        );
        ...
        connection.on(
          ...
          (notification: UnreadCountNotification) => {
            console.log("[SignalR] ReceiveUnreadCountUpdate:", notification);
            handlersRef.current.unreadCountUpdate.forEach((handler) =>
              handler(notification),
            );
          },
        );
  ...
  const onReplyDeleted = useCallback(
    (handler: NotificationHandler<ReplyDeletedNotification>) => {
  ...
  const onAnswerDeleted = useCallback(
    (handler: NotificationHandler<AnswerDeletedNotification>) => {
