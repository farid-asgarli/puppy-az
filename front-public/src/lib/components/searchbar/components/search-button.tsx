 ...
 */
export const SearchButton = ({ isExpanded, activeField, onClick }: SearchButtonProps) => {
  const tAccessibility = useTranslations('accessibility');
  const tCommon = useTranslations('common');

  if (!isExpanded) {
    return (
      <div className="p-[7px_7px_7px_0]">
        <div className="flex items-center justify-center w-8 h-8 rounded-3xl bg-primary text-white">
          <IconSearch size={18} />
        </div>
      </div>
    );
  ...
  return (
    <div className="flex items-center justify-end pr-2">
      ...
      >
        ...
        {activeField && <span className="text-base font-medium whitespace-nowrap">{tCommon('search')}</span>}
      </button>
    </div>
  );
};
