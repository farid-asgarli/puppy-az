 ...
 */
export const PageHeader: React.FC<PageHeaderProps> = ({ title, subtitle, actions, maxWidth = 'md', className }) => {
  const useNarrowContainer = maxWidth === 'md';
  const containerClasses = useNarrowContainer ? '' : cn(maxWidthClasses[maxWidth], 'mx-auto px-6');

  const content = (
    <Row justify="between" align="center">
      {/* Title & Subtitle */}
      <div className="space-y-1 sm:space-y-2">
        <Heading variant="page-title" className="text-2xl sm:text-3xl lg:text-4xl">
          {title}
        </Heading>
