...
const variantStyles: Record<
  CategoryCardVariant,
  { container: string; iconSize: string; iconWrapper: string }
> = {
  full: {
  ...
  pill: {
    container:
      "px-3 py-2 sm:px-4 sm:py-2.5 rounded-xl border-2 border-gray-200 bg-white",
...
export const CategoryCard: React.FC<CategoryCardProps> = ({
    ...
    <>
        ...
        <div className="flex items-center justify-between gap-3 sm:gap-4">
            ...
            <div className="min-w-0">
              <h4
                className={cn(
                  ...
                  isFull ? "text-base sm:text-lg mb-1" : "text-sm sm:text-base",
                )}
              >
                {category.title}
              </h4>
          ...
          {showCount && (
            <div
              ...
              role="status"
              aria-label={t("adsCount", { count: category.petAdsCount })}
            >
              <IconTrendingUp
                size={16}
      ...
      {isPill && (
        <div className="inline-flex items-center gap-1.5 sm:gap-2 font-medium text-gray-900 text-sm sm:text-base">
          <div
            className={cn(styles.iconSize, category.iconColor)}
            dangerouslySetInnerHTML={{ __html: category.svgIcon }}
            aria-hidden="true"
          />
  ...
  return (
    <div className={cn(styles.container, "pointer-events-none", className)}>
      {content}
    </div>
  );
};
