...
import { IconUser, IconMail, IconLock, IconPhone, IconAlertCircle } from '@tabler/icons-react';
...
export function PersonalInfoFormSection({ isLoading, setIsLoading, onNext }: PersonalInfoFormSectionProps) {
  ...
  const getPasswordStrength = (pwd: string): { score: number; label: string; color: string } => {
    ...
    if (score <= 2) return { score, label: t('passwordStrength.weak'), color: 'text-red-600 bg-red-100' };
    if (score <= 3) return { score, label: t('passwordStrength.medium'), color: 'text-yellow-600 bg-yellow-100' };
    return { score, label: t('passwordStrength.strong'), color: 'text-green-600 bg-green-100' };
  };
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <AuthFormField
          ...
          registerProps={register('firstName', {
            ...
            minLength: { value: 3, message: t('validation.firstNameMinLength') },
          })}
        />
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          <div className="absolute left-4 top-1/2 -translate-y-1/2 flex items-center gap-2 text-gray-600 border-r border-gray-200 pr-3">
            <IconPhone size={20} />
            <span className="text-sm font-medium">{t('fields.phonePrefix')}</span>
          </div>
          <input
            id="phoneNumber"
            type="tel"
            ...
            className={cn(
              ...
              errors.phoneNumber ? 'border-red-300 bg-red-50 focus:border-red-500' : 'border-gray-200 bg-white focus:border-gray-900',
      ...
      <div className="space-y-2">
        ...
        {password && passwordStrength.label && (
          <div className={cn('inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-medium', passwordStrength.color)}>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <label htmlFor="acceptTerms" className="text-sm text-gray-600 cursor-pointer">
            ...
            <a href="/terms" target="_blank" className="font-medium text-gray-900 hover:text-primary-600 transition-colors">
              {t('fields.termsOfService')}
            </a>{' '}
            {t('fields.and')}{' '}
            <a href="/privacy" target="_blank" className="font-medium text-gray-900 hover:text-primary-600 transition-colors">
