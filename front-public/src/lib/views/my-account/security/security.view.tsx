...
export default function SecurityView() {
  ...
  const getPasswordStrength = (password: string): { strength: number; label: string; color: string } => {
    ...
    if (strength <= 2) return { strength, label: t('changePassword.strength.weak'), color: 'red' };
    if (strength <= 3) return { strength, label: t('changePassword.strength.medium'), color: 'amber' };
    if (strength <= 4) return { strength, label: t('changePassword.strength.strong'), color: 'green' };
    return { strength, label: t('changePassword.strength.veryStrong'), color: 'green' };
  };
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
        <div className="space-y-6 sm:space-y-8">
          ...
          {submitStatus === 'success' && (
            <StatusMessage variant="success" title={t('messages.success.title')} message={t('messages.success.message')} />
          )}
          ...
          <div className="space-y-4 sm:space-y-6">
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div>
                <h2 className="text-lg sm:text-xl font-semibold text-gray-900">{t('changePassword.title')}</h2>
                <p className="text-sm sm:text-base text-gray-600">{t('changePassword.subtitle')}</p>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 sm:space-y-6">
              ...
              <PasswordInput
                ...
                {...register('confirmPassword', {
                  ...
                  validate: (value) => value === newPassword || t('changePassword.confirmPassword.mustMatch'),
                })}
              />
              ...
              <div className="pt-1 sm:pt-2">
                <Button
                  ...
                  variant="solid"
                  fullWidth
                  leftSection={<IconShield size={18} className="sm:w-5 sm:h-5" />}
                >
