...
const MyAccountView = () => {
  ...
  const quickActions = [
    ...
    },
    {
      ...
      subtitle: tMyAccount('quickActions.favorites.subtitle', { count: userData.stats.favorites }),
      icon: IconHeart,
      href: '/ads/favorites',
    },
    {
      ...
      subtitle: tMyAccount('menu.questions.subtitle', { count: userData.stats.unansweredQuestions }),
      icon: IconMessages,
  ...
  const menuItems: { section: string; items: MenuItem[] }[] = [
    {
      ...
      items: [
        ...
        { icon: IconShield, title: tMyAccount('menu.security.title'), subtitle: tMyAccount('menu.security.subtitle'), href: '/my-account/security' },
        {
          icon: IconBell,
          title: tMyAccount('menu.notifications.title'),
          subtitle: tMyAccount('menu.notifications.subtitle'),
      ...
      items: [
        {
          ...
          subtitle: tMyAccount('menu.myAds.subtitle', { count: userData.stats.totalAds }),
          href: '/my-account/ads',
        },
        {
          icon: IconMessages,
          ...
          subtitle: tMyAccount('menu.questions.subtitle', { count: userData.stats.unansweredQuestions }),
          href: '/my-account/questions',
          badge: userData.stats.unansweredQuestions > 0 ? userData.stats.unansweredQuestions : undefined,
        },
        { icon: IconTrendingUp, title: tMyAccount('menu.stats.title'), subtitle: tMyAccount('menu.stats.subtitle'), href: '/my-ads/stats' },
      ],
    },
    {
    ...
    },
    {
      ...
      items: [
        { icon: IconHelp, title: tMyAccount('menu.helpCenter.title'), subtitle: tMyAccount('menu.helpCenter.subtitle'), href: '/help' },
        {
          icon: IconSettings,
        ...
        <div className="space-y-8 sm:space-y-10 lg:space-y-12">
          ...
          <div className="space-y-6 sm:space-y-8">
            ...
            <div className="flex flex-col sm:flex-row gap-6 items-start">
              ...
              <div className="relative flex-shrink-0 mx-auto sm:mx-0">
                ...
                {userData.isVerified && (
                  <div className="absolute -bottom-1 -right-1 w-6 h-6 sm:w-7 sm:h-7 bg-green-500 rounded-full border-4 border-white flex items-center justify-center">
                    <IconCheck size={12} className="sm:w-[14px] sm:h-[14px] text-white" />
                  </div>
                )}
                <IconButton
                  icon={<IconCamera size={12} className="sm:w-[14px] sm:h-[14px] text-white" />}
              ...
              <div className="flex-1 space-y-3 sm:space-y-4 text-center sm:text-left w-full">
                <div>
                  <Heading variant="subsection" as="h2" className="mb-1 text-xl sm:text-2xl">
                    {userData.name}
                  </Heading>
                  <Text variant="body" color="secondary" className="mb-2 text-sm sm:text-base">
                    {userData.email}
                  </Text>
                  <div className="flex items-center gap-2 justify-center sm:justify-start">
                    <IconMapPin size={14} className="sm:w-4 sm:h-4" />
                    <Text variant="small" color="tertiary" className="text-xs sm:text-sm">
                      {userData.location} • {tMyAccount('memberSince', { date: userData.memberSince })}
                    </Text>
                  </div>
                </div>
                ...
                >
                  ...
                  <Text variant="body" weight="medium" as="span" className="text-sm sm:text-base">
            ...
            {statsLoading ? (
              <div className="grid grid-cols-3 gap-3 sm:gap-4">
                {[1, 2, 3].map((i) => (
                  <div key={i} className="p-4 sm:p-6 rounded-xl border-2 border-gray-200">
              ...
              <div className="grid grid-cols-3 gap-3 sm:gap-4">
                <div className="p-4 sm:p-6 rounded-xl border-2 border-gray-200 text-center">
                  <Heading variant="section" className="mb-1 text-xl sm:text-2xl lg:text-3xl">
              ...
              {quickActions.map((action) => (
                ...
                >
                  <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gray-100 flex items-center justify-center">
                    <action.icon size={20} className="sm:w-6 sm:h-6 text-gray-700" />
                  </div>
                  <div>
                    <Text variant="body" weight="semibold" className="mb-1 text-sm sm:text-base">
                      {action.title}
                    </Text>
                    <Text variant="small" color="secondary" className="text-xs sm:text-sm">
                      {action.subtitle}
          ...
          {menuItems.map((section) => (
            <div key={section.section} className="space-y-4 sm:space-y-6">
              <h2 className="text-lg sm:text-xl font-semibold text-gray-900">{section.section}</h2>
            ...
            >
              <div className="flex items-center gap-3 sm:gap-4">
                <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl bg-red-100 flex items-center justify-center">
                  <IconLogout size={20} className="sm:w-[22px] sm:h-[22px] text-red-600" />
                </div>
                <div className="text-left">
                  <span className="font-semibold text-red-600 block text-sm sm:text-base">{tMyAccount('logout')}</span>
                  <p className="text-xs sm:text-sm text-gray-600">{tMyAccount('logoutDescription')}</p>
                </div>
              </div>
              <IconChevronRight size={18} className="sm:w-5 sm:h-5 text-red-400 flex-shrink-0" />
            </button>
          </div>
        </div>
      </div>
