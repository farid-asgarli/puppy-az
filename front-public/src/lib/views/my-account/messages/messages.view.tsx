...
const MessagesView = () => {
  ...
  const filteredConversations = conversations.filter(
    (conv) =>
      ...
      conv.petAdTitle.toLowerCase().includes(searchQuery.toLowerCase()),
  );
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-16 text-center">
            <div className="max-w-md mx-auto">
              ...
              <Heading
                variant="subsection"
                className="text-gray-900 mb-3 text-xl font-semibold"
              >
...
const ConversationItem = ({
  ...
  return (
    <TransitionLink
      href={`/my-account/messages/${conversation.id}`}
      ...
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-1">
            <div className="flex items-center gap-2">
              <Heading
                variant="subsection"
                as="h4"
                className={cn("text-sm truncate", hasUnread && "font-bold")}
              >
          ...
          <div className="flex items-center gap-1">
            {hasUnread && (
              <span className="flex-shrink-0 w-2 h-2 rounded-full bg-primary-600"></span>
            )}
            <Text
              variant="small"
              className={cn(
        ...
        {conversation.petAdImageUrl && (
          <div className="flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden bg-gray-100">
            <img
              src={`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5005"}${conversation.petAdImageUrl}`}
