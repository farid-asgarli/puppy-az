...
export const AdsSearchView = ({ categories, cities, breeds = null }: AdsSearchViewProps) => {
  const { filters, hasFilters } = useFilterUrl();
  const [totalCount, setTotalCount] = useState(0);
  const [isLoadingResults, setIsLoadingResults] = useState(true);

  // Recent searches hook
  const { recentSearches, addRecentSearch, clearAll, isLoaded } = useRecentSearches({ categories, breeds });

  // Get category title for header
  const categoryTitle = filters.category ? categories.find((c) => c.id === Number(filters.category))?.title || null : null;
  ...
  // Show recent searches only when there are no active filters
  const showRecentSearches = !hasFilters && isLoaded && recentSearches.length > 0;

  return (
    <div className="min-h-screen bg-white pt-24">
      {/* Results Header - Count + Sort */}
      <ResultsHeader totalCount={totalCount} isLoading={isLoadingResults} categoryTitle={categoryTitle} />

      {/* Active Filters Bar - Only shows when filters are active */}
      <ActiveFiltersBar categories={categories} cities={cities} />

      {/* Recent Searches - Shows when no filters are active */}
      {showRecentSearches && (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
          <RecentSearches searches={recentSearches} onClearAll={clearAll} />
        </div>
      )}
