...
import { PetAdType, PetGender, PetSize, CityDto } from '@/lib/api';
...
export const ActiveFiltersBar = ({ categories, cities }: ActiveFiltersBarProps) => {
  ...
  const getFilterLabel = (key: keyof FilterParams, value: string | number): string => {
    switch (key) {
      case 'ad-type':
        return adTypes[value as PetAdType]?.title || value.toString();
      case 'category':
        return categories?.find((c) => c.id === Number(value))?.title || t('category');
      case 'city': {
        const city = cities?.find((c) => c.id === Number(value));
        return city?.name || t('city');
      }
      case 'gender':
        return value === PetGender.Male ? t('gender.male') : value === PetGender.Female ? t('gender.female') : value.toString();
      case 'size':
  ...
  // Get active filters (exclude page and sort, and breed since it's dependent on category)
  const activeFilterEntries = Object.entries(filters).filter(
    ([key, value]) => value !== undefined && value !== null && key !== 'page' && key !== 'sort' && key !== 'breed'
  );
    ...
    <div className="bg-white px-4">
      <NarrowContainer className="mt-8 mb-0 mx-auto border border-gray-200 p-4 sm:px-6 lg:px-8 rounded-xl">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <span className="text-sm font-semibold text-gray-500 flex-shrink-0 hidden sm:block">{t('activeFilters')}:</span>
          ...
          <div className="flex items-center gap-2 flex-wrap flex-1 min-w-0">
            {activeFilterEntries.map(([key, value]) => {
              ...
              return (
                ...
                >
                  <span className="text-sm font-medium text-gray-900 whitespace-nowrap">{label}</span>
                  <button
                    onClick={() => handleRemoveFilter(key as keyof FilterParams)}
                    className={cn(
                  ...
                  >
                    <IconX size={12} className="text-gray-600 group-hover:text-gray-900" />
                  </button>
                </div>
              );
            })}
