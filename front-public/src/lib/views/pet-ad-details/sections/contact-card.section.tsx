...
import { IconUser, IconPhone, IconMail, IconMessageCircle, IconClock } from '@tabler/icons-react';
...
export function AdDetailsContactCardSection({ owner, contactPhone, contactEmail, price, isMobile = false }: AdDetailsContactCardSectionProps) {
      ...
      <div className="space-y-4 sm:space-y-6">
        ...
        {price !== null && (
          <div className="p-4 sm:p-6 rounded-xl border-2 border-gray-200">
            <div className="text-xs sm:text-sm text-gray-600 mb-2">{t('price')}</div>
            <div className="text-2xl sm:text-3xl font-semibold text-gray-900 mb-4">{price.toLocaleString()} ₼</div>
          </div>
        )}
        ...
        <div className="p-4 sm:p-6 rounded-xl border-2 border-gray-200 space-y-4 sm:space-y-6">
          ...
          <div className="space-y-3 sm:space-y-4">
            <h3 className="text-base sm:text-lg font-semibold text-gray-900">{t('contact')}</h3>
            ...
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl bg-gray-100 flex items-center justify-center flex-shrink-0 overflow-hidden">
                {owner.profilePictureUrl ? (
                  <img src={owner.profilePictureUrl} alt={owner.fullName} className="w-full h-full object-cover" />
                ) : (
              ...
              <div className="flex-1 min-w-0">
                <div className="font-semibold text-gray-900 truncate text-sm sm:text-base">{owner.fullName}</div>
          ...
          <div className="space-y-2 sm:space-y-3">
            <Button
              ...
              leftSection={<IconPhone size={16} className="sm:w-[18px] sm:h-[18px]" />}
              onClick={handleCall}
            >
              {t('call')}
            </Button>
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-3">
              <Button
                ...
                leftSection={<IconMessageCircle size={16} className="sm:w-[18px] sm:h-[18px]" />}
              ...
              <Button
                ...
                size="md"
                className="rounded-xl font-medium text-xs sm:text-sm"
                leftSection={<IconMail size={16} className="sm:w-[18px] sm:h-[18px]" />}
                onClick={handleEmail}
              >
                {t('email')}
              </Button>
          ...
          <div className="pt-3 sm:pt-4 border-t-2 border-gray-200 space-y-2 sm:space-y-3">
            <div className="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm">
              <IconPhone size={14} className="sm:w-4 sm:h-4 text-gray-400 flex-shrink-0" />
              <span className="text-gray-700 truncate">{contactPhone}</span>
            </div>
            <div className="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm">
              <IconMail size={14} className="sm:w-4 sm:h-4 text-gray-400 flex-shrink-0" />
    ...
    <div className="fixed bottom-0 left-0 right-0 bg-white border-t-2 border-gray-200 p-4 sm:p-6 z-20">
      <div className="flex items-center justify-between gap-3 sm:gap-4">
        <div className="flex-1 min-w-0">
          {price !== null ? (
            <>
              <div className="text-xl sm:text-2xl font-semibold text-gray-900">{price.toLocaleString()} ₼</div>
              <div className="text-xs sm:text-sm text-gray-600">{t('price')}</div>
            </>
          ) : (
            <>
              <div className="text-base sm:text-lg font-semibold text-gray-900">{owner.fullName}</div>
              <div className="text-xs sm:text-sm text-gray-600">{t('seller')}</div>
            </>
          )}
        </div>
