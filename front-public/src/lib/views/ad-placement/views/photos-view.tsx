    ...
    async (files: FileList | null) => {
      ...
      for (const file of validFiles) {
        try {
          ...
          console.log(
            `Uploading ${file.name} (${file.size} bytes, ${file.type})`,
          );

          // Upload directly to API with locale - native fetch handles File objects perfectly
          const result = await petAdService.uploadPetAdImage(
            file,
            token,
            locale,
          );
        ...
        <div className="space-y-8">
          ...
          <div className="flex items-center justify-between">
            <p className="text-sm font-medium text-gray-700">
              ...
              {uploading && (
                <span className="ml-2 text-gray-500">({t("uploading")})</span>
              )}
            </p>
            {uploadedImages.length >= minPhotos && (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {uploadedImages.map((image, index) => (
                ...
                >
                  ...
                  {/* Primary Badge */}
                  {index === 0 && (
                    <div className="absolute top-2 left-2 px-2 py-1 bg-black text-white text-xs font-semibold rounded-lg z-10">
                      {t("primary")}
                    </div>
                  )}
          ...
          {uploadedImages.length < maxPhotos && !uploading && (
            ...
            >
              ...
              <Text variant="small" color="muted">
                {t("uploadRemaining", {
                  count: maxPhotos - uploadedImages.length,
                  plural: maxPhotos - uploadedImages.length === 1 ? "" : "s",
                })}
              </Text>
            </div>
          )}
          ...
          {uploading && Object.keys(uploadProgress).length > 0 && (
            <div className="flex items-center justify-center gap-3 p-4 bg-info-50 border border-info-200 rounded-xl">
              <svg
                className="animate-spin h-5 w-5 text-info-600"
              ...
              <p className="text-sm text-info-700 font-medium">
                {t("uploadingProgress", {
                  count: Object.keys(uploadProgress).length,
                })}
              </p>
            </div>
          )}
          ...
          <div className="bg-gray-50 rounded-xl p-6 space-y-3">
            <Heading variant="subsection">{t("tipsHeading")}</Heading>
