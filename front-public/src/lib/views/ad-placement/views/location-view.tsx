...
export default function LocationView() {
  ...
  const [selectedCityId, setSelectedCityId] = useState<number | null>(formData.cityId);
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-2">
            ...
            <Text variant="body-lg" color="secondary">
              {cities.length > 0 ? t('subtitle', { count: cities.length }) : t('noCities')}
            </Text>
          </div>

          {/* Search Input */}
          {cities.length > 0 && <SearchInput value={searchQuery} onChange={setSearchQuery} placeholder={t('searchPlaceholder')} />}
          ...
          {searchQuery && (
            <Text variant="small" color="secondary">
              {filteredCities.length === 0
                ? t('noResults')
                : t('resultsCount', { count: filteredCities.length, plural: filteredCities.length !== 1 ? 'cities' : 'city' })}
            </Text>
          )}
          ...
          <div className="space-y-3 max-h-[500px] overflow-y-auto">
            {filteredCities.length === 0 && searchQuery ? (
              <div className="text-center py-12">
                <p className="text-gray-500 text-lg">{t('noMatch', { query: searchQuery })}</p>
