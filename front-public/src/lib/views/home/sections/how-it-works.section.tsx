...
import { IconSearch, IconMessageCircle, IconHeart, IconShieldCheck, IconSparkles } from '@tabler/icons-react';
  ...
  const steps = [
    {
      ...
      gradient: 'from-info-500 via-info-600 to-info-600',
    ...
    },
    {
      ...
      gradient: 'from-primary-500 via-primary-600 to-accent-600',
    ...
    },
    {
      ...
      gradient: 'from-accent-500 via-accent-600 to-error-600',
        ...
        <div className="space-y-10 sm:space-y-12 lg:space-y-16">
          ...
          <div className="text-center space-y-4">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-info-50 to-primary-50 border-2 border-info-100 rounded-full">
              ...
              <span className="text-sm font-semibold text-info-700">{t('badge')}</span>
            </div>
              ...
              return (
                <div key={step.number} className="relative group">
                  ...
                  >
                    ...
                    >
                      <Icon size={32} className={cn(step.iconColor, 'drop-shadow-sm')} strokeWidth={2} />
                    </div>

                    {/* Content */}
                    <div className="space-y-3">
                      <h3 className="text-lg sm:text-xl font-bold font-heading text-gray-900 leading-snug">{step.title}</h3>
                      <p className="text-sm sm:text-base text-gray-600 leading-relaxed">{step.description}</p>
                    </div>

                    {/* Bottom accent line */}
                    <div className={cn('h-1 w-12 rounded-full bg-gradient-to-r', step.gradient, 'opacity-60')} />
                  </div>
                </div>
              );
            })}
          ...
          <div className="max-w-3xl mx-auto">
            <div className="bg-gradient-to-br from-gray-50 to-white border-2 border-gray-100 rounded-2xl p-6 sm:p-8 text-center space-y-6">
              <p className="text-base sm:text-lg font-medium text-gray-700">{t('readyMessage')}</p>
              <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-6">
                <div className="flex items-center gap-2 px-4 py-2 bg-white rounded-full border border-gray-200 shadow-sm">
                  <IconShieldCheck size={18} className="text-green-600" strokeWidth={2.5} />
                  <span className="text-sm font-medium text-gray-700">{t('secure')}</span>
                </div>
                <div className="flex items-center gap-2 px-4 py-2 bg-white rounded-full border border-gray-200 shadow-sm">
                  <IconHeart size={18} className="text-accent-600 fill-accent-600" strokeWidth={2.5} />
                  <span className="text-sm font-medium text-gray-700">{t('verifiedSellers')}</span>
                </div>
                <div className="flex items-center gap-2 px-4 py-2 bg-white rounded-full border border-gray-200 shadow-sm">
                  <IconMessageCircle size={18} className="text-info-600" strokeWidth={2.5} />
                  <span className="text-sm font-medium text-gray-700">{t('support')}</span>
                </div>
              </div>
            </div>
          </div>
