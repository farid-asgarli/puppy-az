...
import type { PetCategoryWithAdsDto, PetAdListItemDto } from '@/lib/api/types/pet-ad.types';
...
export default async function Page() {
  ...
  try {
    const [_categories, categoriesResult, premiumAdsResponse] = await Promise.all([
      petAdService.getPetCategoriesDetailed(locale).catch(() => []),
      petAdService.getPetCategoriesWithAds(locale).catch(() => []),
      petAdService.getPremiumAds({ pagination: { number: 1, size: 10 } }, locale).catch(() => ({ items: [], totalCount: 0 })),
    ]);
